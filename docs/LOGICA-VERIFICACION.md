...
En el modulo de operaciones pendientes no se muestra el monto en Bs de la operación, corrigelo y hazme un archivo con las correcciones del sql si hace falta. y en el modulo de perfil, si el usuario no tiene ninguna imagen o solo está cargando, que se coloque la imagen user1.jpeg, para que no se deforme el contenedor. y para la pantalla de retiro, quiero que en la base de datos se guarden las cuentas bancarias que el usuario tenga registradas, esta tabla cuenta_cuenta bancaria debe hacer referencia a otra tabla llamada banco, la cual tendrá banco_id, nombre_banco, codigo_banco. el formulario debe estar pero solo para guardar una cuenta bancaria.